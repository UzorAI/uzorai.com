...
`npm run test:language:browser` crawls the production build for every shipped
locale (`en`, `es`, `ru`, `zh`, `ar`, `he`, `fr`, and `uk`) across both host
roles, all routes and conditional states, desktop/mobile, and both themes. The
CI workflow uploads screenshots and audit snapshots. Run `npm run build` first.
